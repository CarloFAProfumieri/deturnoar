...
def getTurnos(turnos):
    ...
    for i, turno in enumerate(turnos, start=1):
        ...
        img = thick_font(img)
        # show(img)
        ocr_result = pytesseract.image_to_string(img, lang='spa', config='--psm 12 --oem 3')
        turnos_text_raw.append(ocr_result)
...
for c in cnts:
    ...
    if 150 < w < 500 and 150 < h < 1000:  # Ignore very small or too large boxes
        ...
        if not is_duplicate:
            ...
            y_line = getYpoint(snippet)  # This function returns a y-coordinate to draw the line

            cv2.line(turno_crop, (0, y_line), (turno_crop.shape[1], y_line), (0, 255, 0), 2)  # Green line
...
for unt in turnos_clean_santafe:
    cv2.imshow("turno",unt)
    cv2.waitKey(0)
