...
function addSearchListener(){
    document.addEventListener("DOMContentLoaded", function () {
        ...
        searchForm.addEventListener("submit", function (event) {
            ...
            if (addressInput) {
                let searchInput = addressInput + ", SANTO TOME, SANTA FE, ARGENTINA"
...
let map = new maplibregl.Map({
    ...
    container: 'map',
    center: [-60.770281633592134, -31.67461803741591],
    zoom: 12.7,
})
...
Promise.all([
    fetch("data/turnos-santo-tome.json").then(res => res.json()),
